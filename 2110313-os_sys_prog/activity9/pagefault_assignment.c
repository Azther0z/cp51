...
int get_free_frame(int page_number, int timestamp) {
    ...
    // If no free frame, select one of occupied frames using the chosen algorithm
        ...
        int page_to_remove = frames[oldest_frame].page_number;
        page_table[page_to_remove].valid = 0;
        // Assignment 1.4
    ...
    while (token != NULL) {
        ...
        if (page_table[page_number].valid == 0) {
            ...
            if (use_lru) {
                ...
                int target_frame = page_table[page_number].frame;
                frames[target_frame].timestamp = page_references;
            }
