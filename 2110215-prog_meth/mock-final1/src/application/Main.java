...
public class Main {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			case 5:
				...
				while (true) {
					System.out.print(
							String.format("Select Processor Unit to be removed (1 - %d): ", processUnits.size()));
					int index = 0;
					try {
	...
	public static void estimateTask() {
		...
		for (int ck = 1; ck <= TaskList.getTasks().size(); ck++) {
			...
			for (int j = 0; j < 4; j++) {
				...
				case 2:
					...
					break;
				case 3:
					addString += "Instruction Count: " + TaskList.getTask(ck - 1).getWorkload()
		...
		System.out.println(
				"Estimated task time for parallel computing: " + processUnit.parallelCompute(task) + " second");
	}
	...
	public static void modifyTask() {
		...
		case 3:
			while (true) {
				try {
					System.out.printf("Index of task to be merge with selected task(1 - %d, not %d): ", tasks.size(),
							taskIndex);
