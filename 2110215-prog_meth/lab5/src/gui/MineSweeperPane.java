...
public class MineSweeperPane extends GridPane{
	...
	public MineSweeperPane() {
		...
		Border bd = new Border(new BorderStroke(Color.LIGHTGRAY,BorderStrokeStyle.SOLID,CornerRadii.EMPTY,BorderWidths.DEFAULT));
		this.setBorder(bd);
		Background bg = new Background(new BackgroundFill(Color.WHITE, CornerRadii.EMPTY, Insets.EMPTY));
		this.setBackground(bg);
		for(int i=0;i<5;i++) {
			for(int j=0;j<5;j++){
				MineSweeperSquare mss = new MineSweeperSquare(i,j);
				this.allCells.add(mss);
				this.add(mss, i, j);
			}
		}
	}
