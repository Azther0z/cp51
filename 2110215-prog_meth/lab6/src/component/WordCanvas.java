		...
		Thread t = new Thread(() -> {
			try {
				for (double i = 21; i > 0; i--) {
					...
					Platform.runLater(() -> {
						updateCanvas(); // CAUTION: This method contains UI update, which can cause an error if running
		...
		Thread t = new Thread(()->{
			try {
				for (int i = 0; i < 5; i++) {
					int index = i;
//					new Thread(()->{
//						flipLetter(index, newStatus[index]);
//						try {
//							Thread.sleep(300);
//						} catch (InterruptedException e) {
...
//					}).start();
				...
				Thread.sleep(1000);
				Platform.runLater(()->{
					GameLogic.endTurns(); // CAUTION: This method contains UI update, which can cause an error if running
					// in a different thread
				});
